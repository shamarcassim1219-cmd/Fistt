...
class _VerificationScreenState extends State<VerificationScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final nicUrl = await CloudinaryService.uploadImage(_nicImage!, folder: 'verifications');
      final selfieUrl = await CloudinaryService.uploadImage(_selfieImage!, folder: 'verifications');

      await ApiService.submitVerification(nicUrl, selfieUrl);
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: const [
                        Text('Get the Blue Checkmark', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.white)),
                        SizedBox(height: 4),
                        Text('One-time fee: LKR 150. Verified sellers get more trust and higher sales.',
            ...
            _UploadBox(image: _nicImage, icon: Icons.badge_outlined, label: 'Tap to upload NIC photo', onTap: () => _pickImage(true)),
            const SizedBox(height: 24),
            const _StepLabel('2. Take a selfie'),
            const SizedBox(height: 8),
            _UploadBox(image: _selfieImage, icon: Icons.camera_alt_outlined, label: 'Tap to take a selfie', onTap: () => _pickImage(false)),
            const SizedBox(height: 24),
            ...
            Container(
              ...
              child: const Text(
                'Payment is confirmed by admin after reviewing your bank transfer slip. Your badge appears in Settings once approved.',
                style: TextStyle(fontSize: 12, color: AppColors.hint),
              ),
            ),
            if (_error != null) ...[
