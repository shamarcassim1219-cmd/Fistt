...
class _AddListingScreenState extends State<AddListingScreen> {
  ...
  Future<List<String>> _uploadScreenshots(String listingId) async {
    ...
    for (var i = 0; i < _screenshots.length; i++) {
      final ref = FirebaseStorage.instance
          .ref('listings/$listingId/screenshot_$i.jpg');
      await ref.putFile(_screenshots[i]);
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // 1. Public listing document — safe to read by anyone
      ...
      // 2. Vault credentials — sent to a callable Cloud Function that encrypts
      // and stores them server-side. The client NEVER writes plain credentials
      // directly to Firestore. Replace this with your actual callable once
      // the Cloud Function (encryptAndStoreVault) is deployed — see
      // firestore/schema.md, Cloud Functions section.
      //
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              _SectionLabel('Screenshots (max 6)'),
              GridView.builder(
                shrinkWrap: true,
              ...
              _SectionLabel('Account Vault (Private & Encrypted)'),
              Container(
  ...
  Widget _row(String label, String value, {bool bold = false}) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(label, style: TextStyle(fontSize: 12, fontWeight: bold ? FontWeight.bold : null)),
          Text(value, style: TextStyle(fontSize: 12, fontWeight: bold ? FontWeight.bold : null)),
        ],
      ),
    );
  }
