...
class _SaleDetailScreenState extends State<SaleDetailScreen> with SecureScreenMixin {
  ...
  String _formatDuration(Duration d) {
    if (d.inSeconds <= 0) return AppLocalizations.t('releasing_soon');
    final days = d.inDays;
    final hours = d.inHours % 24;
    final minutes = d.inMinutes % 60;
    if (days > 0) {
      return '${days}d ${hours}h ${minutes}m ${AppLocalizations.t('until_payout')}';
    }
    final h = d.inHours;
    final m = d.inMinutes % 60;
    final s = d.inSeconds % 60;
    return '${h.toString().padLeft(2, '0')}:${m.toString().padLeft(2, '0')}:${s.toString().padLeft(2, '0')} ${AppLocalizations.t('until_payout')}';
  }

  String _statusLabel(String status) {
    final map = {
      'escrow_held': AppLocalizations.t('in_escrow'),
      'completed': AppLocalizations.t('paid_out'),
    ...
    return ValueListenableBuilder<String>(
      ...
      builder: (context, lang, _) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(title: Text('${AppLocalizations.t('my_sales')} Details')),
          body: ListView(
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    _row(AppLocalizations.t('order_id'), '#${o['id']}'),
                    _row(AppLocalizations.t('sale_price'), 'LKR ${(o['price'] as num).toStringAsFixed(2)}'),
                    _row(AppLocalizations.t('you_receive_amount'), 'LKR ${(o['sellerPayout'] as num).toStringAsFixed(2)}'),
                    _row(AppLocalizations.t('status'), _statusLabel(status)),
                    if (o['createdAt'] != null)
                      _row(AppLocalizations.t('sold_on'), DateTime.parse(o['createdAt']).toLocal().toString().substring(0, 16)),
                  ],
                ),
              ),
              ...
              if (status == 'escrow_held') ...[
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(child: Text(AppLocalizations.t('payment_released'), style: const TextStyle(color: Colors.greenAccent, fontSize: 13))),
                    ],
                  ),
                )
                ...
                Container(
                  ...
                  child: Text(AppLocalizations.t('sale_under_dispute'), style: const TextStyle(color: Colors.redAccent, fontSize: 13)),
                ),
            ],
          ),
        );
